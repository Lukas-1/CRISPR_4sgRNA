...
NCBI_directory          <- file.path(CRISPR_input_directory, "Mouse genome", "NCBI")
annotation_intermediate_files_directory <- file.path(CRISPR_root_directory, "4) Intermediate files", "Mouse - Annotation")
...
# Downloaded from ftp://ftp.ensembl.org/pub/release-100/tsv/mus_musculus/Mus_musculus.GRCm38.100.entrez.tsv.gz
# on 6 October 2020
Ensembl_Mm_entrez_df <- read.table(file.path(CRISPR_input_directory, "Mouse genome", "Ensembl", "Mus_musculus.GRCm38.100.entrez.tsv"),
...
Ensembl_Mm_are_protein_coding <- Ensembl_Mm_entrez_df[["protein_stable_id"]] != "-"
Ensembl_entrezs_vec <- unique(Ensembl_Mm_entrez_df[["xref"]][Ensembl_Mm_have_entrez & Ensembl_Mm_are_protein_coding])
...
collected_entrezs_df <- data.frame(
  ...
  "Is_in_NCBI_Mm_info"      = collected_entrez_IDs %in% NCBI_entrezs_vec,
...
collected_entrezs_df[["Category"]] <- vapply(seq_along(collected_entrez_IDs), function(x) {
  is_in_NCBI_info <- collected_entrezs_df[["Is_in_NCBI_Mm_info"]][[x]]
...
if (!(all(collected_entrezs_df[["Is_in_NCBI_gene"]]))) {
  ...
  search_term <- paste0("https://www.ncbi.nlm.nih.gov/gene/?term=%22Mus+musculus",
