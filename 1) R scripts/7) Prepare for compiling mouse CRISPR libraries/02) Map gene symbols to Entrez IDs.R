...
# Downloaded from ftp://ftp.ncbi.nlm.nih.gov/gene/DATA/GENE_INFO/Mammalia/Mus_musculus.gene_info.gz
# on 6 October 2020

NCBI_Mm_info_df <- read.table(file.path(CRISPR_input_directory, "Mouse genome", "NCBI", "Mus_musculus.gene_info"),
...
Entrez_map_df <- data.frame(
  ...
  "Symbol_Org_Mm_eg_db" = EntrezIDsToSymbols(unique_entrez_IDs),
  "Symbol_NCBI_Mm_info" = NCBI_Mm_info_df[["Symbol"]][match(unique_entrez_IDs, NCBI_Mm_info_df[["GeneID"]])],
  "Chromosome"          = chromosome_vec,
  stringsAsFactors = FALSE
)

are_identical <- mapply(identical, Entrez_map_df[["Symbol_Org_Mm_eg_db"]], Entrez_map_df[["Symbol_NCBI_Mm_info"]])

Entrez_map_df[!(are_identical), ]

Entrez_map_df[is.na(Entrez_map_df[["Symbol_Org_Mm_eg_db"]]) & !(is.na(Entrez_map_df[["Symbol_NCBI_Mm_info"]])), ]

Entrez_map_df[!(are_identical) & !(is.na(Entrez_map_df[["Symbol_Org_Mm_eg_db"]])) & !(is.na(Entrez_map_df[["Symbol_NCBI_Mm_info"]])), ]
...
# Create a list of gene synonyms from NCBI's Mm.gene_info -----------------

are_empty <- NCBI_Mm_info_df[["Synonyms"]] == "-"

synonyms_list <- strsplit(NCBI_Mm_info_df[["Synonyms"]][!(are_empty)], "|", fixed = TRUE)

entrez_to_synonyms_list <- synonyms_list
names(entrez_to_synonyms_list) <- NCBI_Mm_info_df[["GeneID"]][!(are_empty)]

synonyms_to_entrez_list <- ReverseAndRelist(entrez_to_synonyms_list)

# symbol_to_synonyms_list <- synonyms_list
# names(symbol_to_synonyms_list) <- NCBI_Mm_info_df[["Symbol"]][!(are_empty)]
# synonyms_to_symbol_list <- ReverseAndRelist(symbol_to_synonyms_list)




# Convert the lists from NCBI's Mm.info file to all capital letters -------
...
collected_symbols <- sort(unique(c(names(symbol_to_entrez_list),
                                   ...
                                   toupper(NCBI_Mm_info_df[["Symbol"]]),
...
symbols_df <- data.frame("Symbol"            = collected_symbols,
                         ...
                         "Number_OrgMm"      = vapply(collected_symbols, function(x) if (x %in% names(symbol_to_entrez_list)) length(symbol_to_entrez_list[[x]]) else NA_integer_, integer(1)),
                         "Number_all"        = lengths(combined_list),
                         "Entrez_IDs_OrgMm"  = vapply(collected_symbols, function(x) if (x %in% names(symbol_to_entrez_list)) paste0(symbol_to_entrez_list[[x]], collapse = ", ") else NA_character_, ""),
...
unique_OrgMm_symbols_list <- symbol_to_entrez_list[lengths(symbol_to_entrez_list) == 1]
unique_OrgMm_alias_list   <- alias_to_entrez_list[lengths(alias_to_entrez_list) == 1]
unique_NCBI_synonyms_list <- synonyms_to_entrez_list[lengths(synonyms_to_entrez_list) == 1]

intersect_OrgMm_NCBI_list <- sapply(intersect(names(alias_to_entrez_list), names(synonyms_to_entrez_list)),
                                    function(x) intersect(alias_to_entrez_list[[x]], synonyms_to_entrez_list[[x]])
                                    , simplify = FALSE
                                    )
unique_intersect_OrgMm_NCBI_list <- intersect_OrgMm_NCBI_list[lengths(intersect_OrgMm_NCBI_list) == 1]

symbols_df[["Unique_in_OrgMm_symbol"]] <- vapply(collected_symbols, function(x) if (x %in% names(unique_OrgMm_symbols_list)) unique_OrgMm_symbols_list[[x]] else NA_character_, "")
symbols_df[["Present_in_NCBI_symbol"]] <- as.character(NCBI_Mm_info_df[["GeneID"]][match(collected_symbols, toupper(NCBI_Mm_info_df[["Symbol"]]))])
symbols_df[["Unambiguous_alias"]] <- vapply(collected_symbols, function(x) {
  unique_OrgMm_alias_lookup <- unique_OrgMm_alias_list[[x]]
  unique_NCBI_synonyms_lookup <- unique_NCBI_synonyms_list[[x]]
  if (is.null(unique_OrgMm_alias_lookup) || is.null(unique_NCBI_synonyms_lookup)) {
    return(NA_character_)
  } else {
    unique_symbols <- unique(unlist(c(unique_OrgMm_alias_lookup, unique_NCBI_synonyms_lookup), use.names = FALSE))
...
symbols_df[["Unique_in_overlap"]]      <- vapply(collected_symbols, function(x) if (x %in% names(unique_intersect_OrgMm_NCBI_list)) unique_intersect_OrgMm_NCBI_list[[x]] else NA_character_, "")
symbols_df[["Unique_in_OrgMm_alias"]]  <- vapply(collected_symbols, function(x) if (x %in% names(unique_OrgMm_alias_list)) unique_OrgMm_alias_list[[x]] else NA_character_, "")
...
symbols_df[["Min_OrgMm_symbol"]]  <- vapply(collected_symbols, function(x) {
...
symbols_df[["Min_OrgMm_alias"]]  <- vapply(collected_symbols, function(x) {
...
symbols_assignment_map <- c(
  "Unique_in_OrgMm_symbol" = "Only one mapping in Org.Mm.egSYMBOL",
  "Present_in_NCBI_symbol" = "No unique mapping in Org.Mm.egSYMBOL, but found in the Symbol column of Mus_musculus.gene_info",
  "Unambiguous_alias"      = "One and only one mapping in both Org.Mm.egALIAS and Mus_musculus.gene_info",
  "Unique_in_overlap"      = "Only one mapping that is common to both Org.Mm.egALIAS and Mus_musculus.gene_info",
  "Unique_in_OrgMm_alias"  = "Only present in Org.Mm.egALIAS, and only one mapping found",
  "Unique_in_NCBI_synonym" = "Only present in Mus_musculus.gene_info, and only one mapping found",
  "Min_OrgMm_symbol"       = "Maps to multiple IDs in Org.Mm.egSYMBOL; the first (lowest) ID was chosen",
  "Min_OrgMm_alias"        = "Maps to multiple IDs in Org.Mm.egALIAS; the first (lowest) ID was chosen",
  "Min_NCBI_synonym"       = "Not found in Org.Mm.egALIAS, but maps to multiple IDs in Mus_musculus.gene_info; the first (lowest) ID was chosen"
)
