...
load(file.path(CRISPRi_RData_directory, "15) Separate sgRNAs for genes with multiple relevant TSSs.RData"))
...
missing_offtargets_df <- AddCRISPORBedData(merged_replaced_CRISPRi_df, CRISPOR_bed_df, CRISPOR_offtargets_bed_df,
...
merged_replaced_CRISPRi_df <- AddCRISPORBedData(merged_replaced_CRISPRi_df,   CRISPOR_bed_df,   CRISPOR_offtargets_bed_df)
# merged_replaced_CRISPRi_df <- AddCRISPORFASTAData(merged_replaced_CRISPRi_df, CRISPOR_FASTA_df, CRISPOR_offtargets_FASTA_df)
...
merged_replaced_CRISPRi_df[["PAM_0MM"]] <- TruncateLongEntries(merged_replaced_CRISPRi_df[["PAM_0MM"]])
...
both_are_available <- !(is.na(merged_replaced_CRISPRi_df[["CRISPOR_CFD_specificity"]])) &
                      !(is.na(merged_replaced_CRISPRi_df[["GuideScan_specificity"]]))
both_are_available <- both_are_available & !(is.na(merged_replaced_CRISPRi_df[["CRISPOR_3MM_specificity"]]))

GetProportion(merged_replaced_CRISPRi_df[["CRISPOR_CFD_specificity"]][both_are_available] >= 80)
GetProportion(merged_replaced_CRISPRi_df[["GuideScan_specificity"]][both_are_available] >= 0.2)
GetProportion(merged_replaced_CRISPRi_df[["CRISPOR_3MM_specificity"]][both_are_available] >= 0.2)
GetProportion(merged_replaced_CRISPRi_df[["CRISPOR_4MM_specificity"]][both_are_available] >= 0.04)
...
only_GuideScan_present <- !(is.na(merged_replaced_CRISPRi_df[["GuideScan_specificity"]])) &
                            is.na(merged_replaced_CRISPRi_df[["CRISPOR_3MM_specificity"]])
...
if (any(only_GuideScan_present)) {
  ...
  print(merged_replaced_CRISPRi_df[only_GuideScan_present, specificity_demo_columns])
}
...
save(list = "merged_replaced_CRISPRi_df",
     file = file.path(CRISPRi_RData_directory, "17) Integrate the output from CRISPOR.RData")
     )
