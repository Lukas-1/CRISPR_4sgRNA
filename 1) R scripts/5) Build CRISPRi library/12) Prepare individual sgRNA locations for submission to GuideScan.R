...
were_not_searched <- !(merged_replaced_CRISPRi_df[["TSS_searched_by_GuideScan"]] %in% "Yes")
were_mapped <- !(is.na(merged_replaced_CRISPRi_df[["Start"]]))

submit_df <- merged_replaced_CRISPRi_df[were_not_searched & were_mapped, ]
...
choose_columns <- c("Entrez_ID", "Gene_symbol", "Original_symbol", "Source", "hCRISPRi_v2_transcript",
                    "sgRNA_sequence", "PAM", "Dolcetto_rank", "GPP_rank", "hCRISPRi_v2_rank",
                    "Chromosome", "Strand", "Start", "End",
                    "Distance_from_TSS", "TSS_searched_by_GuideScan",
                    "GuideScan_efficiency", "GuideScan_specificity",
                    "Num_0MM", "Num_5G_MM", "Num_1MM", "GuideScan_Num_2MM",
                    "GuideScan_Num_3MM", "GuideScan_offtarget_category",
...
write.table(GuideScan_input_vec,
            file = file.path(GuideScan_files_directory, "Input_for_GuideScan_CRISPRi_individual_locations.txt"),
