...
load(file.path(CRISPRi_RData_directory, "06) Find matches for sgRNA sequences in the human genome - all_sequences_df.RData"))
...
replaced_unique_sequences <- unique(toupper(replaced_merged_CRISPRi_df[["sgRNA_sequence"]]))
new_or_not_found_sequences <- setdiff(replaced_unique_sequences, all_sequences_df[["Reference"]])

rm(replaced_merged_CRISPRi_df)

# All replaced sequences should be from the hCRISPRi-v2 library and should be 20-mers.
...
save(list = "replaced_all_sequences_df",
     file = file.path(CRISPRi_RData_directory, "10) Find matches for sgRNA sequences after fixing 5'G substitutions - replaced_all_sequences_df.RData")
     )
save(list = "replaced_genome_search_df",
     file = file.path(CRISPRi_RData_directory, "10) Find matches for sgRNA sequences after fixing 5'G substitutions - replaced_genome_search_df.RData")
     )
