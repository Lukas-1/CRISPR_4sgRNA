...
# Process the genomic locations of sgRNAs from hCRISPRi-v2 ----------------

have_locations <- !(is.na(CRISPRi_df[["hCRISPRi_v2_ID"]])) & (CRISPRi_df[["Is_control"]] == "No")

hCRISPRi_v2_IDs_vec <- CRISPRi_df[["hCRISPRi_v2_ID"]][have_locations]

have_two_entries <- grepl(";", hCRISPRi_v2_IDs_vec, fixed = TRUE)
two_entries_splits <- strsplit(hCRISPRi_v2_IDs_vec[have_two_entries], "; ", fixed = TRUE)
hCRISPRi_v2_IDs_vec[have_two_entries] <- sapply(two_entries_splits, "[[", 1)

hCRISPRi_v2_IDs_second_half <- sapply(strsplit(hCRISPRi_v2_IDs_vec, "_[-+]_"), "[[", 2)
hCRISPRi_v2_location_vec <- as.integer(sapply(strsplit(hCRISPRi_v2_IDs_second_half, "-", fixed = TRUE), "[[", 1))

hCRISPRi_v2_ID_splits <- strsplit(hCRISPRi_v2_IDs_vec, "_", fixed = TRUE)
hCRISPRi_v2_strand_vec <- vapply(hCRISPRi_v2_ID_splits, function(x) x[x %in% c("-", "+")], "")
...
unique_entrez_ID_strings <- unique(CRISPRi_df[["Entrez_ID"]][have_locations])
...
locations_short_df <- data.frame(
  CRISPRi_df[have_locations, c("Combined_ID", "Entrez_ID", "Gene_symbol", "Original_symbol", "hCRISPRi_v2_transcript")],
  "Original_sequence" = CRISPRi_df[["sgRNA_sequence"]][have_locations],
  "Location"          = hCRISPRi_v2_location_vec,
  "Strand"            = c("+" = "-", "-" = "+")[hCRISPRi_v2_strand_vec],
...
selected_columns <- c("hCRISPRi_v2_transcript", "Gene_symbol", "Overlapping_symbols", "Original_symbol",
...
unique(CRISPRi_df[is.na(CRISPRi_df[["Entrez_ID"]]) & have_locations, c("Original_symbol", "hCRISPRi_v2_transcript")])
...
per_gene_columns <- c("Entrez_ID", "Gene_symbol", "Original_symbol", "hCRISPRi_v2_transcript", "Chromosome")
...
# Modify CRISPRi_df for sgRNAs from hCRISPRi-v2 ---------------------------

CRISPRi_df_IDs <- paste0(CRISPRi_df[["Combined_ID"]][have_locations], "__", CRISPRi_df[["sgRNA_sequence"]][have_locations])
lifted_df_IDs <- paste0(confirmed_lifted_df[["Combined_ID"]], "__", confirmed_lifted_df[["Original_sequence"]])

ID_matches <- match(CRISPRi_df_IDs, lifted_df_IDs)

unique(CRISPRi_df[have_locations, ][is.na(ID_matches), c("Entrez_ID", "Gene_symbol", "Original_symbol", "hCRISPRi_v2_transcript")])


new_entrezs_vec <- ifelse(is.na(ID_matches),
                          CRISPRi_df[["Entrez_ID"]][have_locations],
                          confirmed_lifted_df[["Entrez_ID"]][ID_matches]
                          )

CRISPRi_df[["Entrez_ID"]][have_locations] <- new_entrezs_vec
CRISPRi_df[["Combined_ID"]][have_locations] <- ifelse(is.na(CRISPRi_df[["Entrez_ID"]][have_locations]),
                                                    toupper(CRISPRi_df[["Original_symbol"]][have_locations]),
                                                    CRISPRi_df[["Entrez_ID"]][have_locations]
                                                    )
...
if (any(are_to_replace_symbols)) {
  new_symbols_vec <- CRISPRi_df[["Gene_symbol"]][have_locations]
  new_symbols_vec[are_to_replace_symbols] <- MapToEntrezs(entrez_IDs_vec = new_entrezs_vec[are_to_replace_symbols])[["Gene_symbol"]]
  CRISPRi_df[["Gene_symbol"]][have_locations] <- new_symbols_vec
  CRISPRi_df[["Original_symbol"]][have_locations][are_to_replace_symbols]
}

new_sequences_vec <- ifelse(is.na(ID_matches),
                            CRISPRi_df[["sgRNA_sequence"]][have_locations],
                            paste0(substr(confirmed_lifted_df[["Sequence_hg19"]][ID_matches], 1, 1),
                                   substr(CRISPRi_df[["sgRNA_sequence"]][have_locations], 2, 20)
                                   )
                            )
CRISPRi_df[["sgRNA_sequence"]][have_locations] <- new_sequences_vec

CRISPRi_df[["Exchanged_5pG"]] <- NA_character_
CRISPRi_df[["Exchanged_5pG"]][have_locations] <- ifelse(is.na(ID_matches), "No", "Yes")

CRISPRi_df[["Location_liftOver"]] <- NA_character_
CRISPRi_df[["Location_liftOver"]][have_locations] <- confirmed_lifted_df[["Location_liftOver"]][ID_matches]


CRISPRi_df[["Original_PAM"]][have_locations] <- ifelse(is.na(CRISPRi_df[["Original_PAM"]][have_locations]),
                                                       confirmed_lifted_df[["PAM_liftOver"]][ID_matches],
                                                       CRISPRi_df[["Original_PAM"]][have_locations]
                                                       )
...
CRISPRi_df <- ResolveDuplicates(CRISPRi_df, concatenate_columns = c("Sublibrary", "hCRISPRi_v2_ID", "hCRISPRi_TSS_source"))
...
save(list = "CRISPRi_df",
     file = file.path(CRISPRi_RData_directory, "02) Extract the original sequences for sgRNAs from hCRISPRi-v2 - CRISPRi_df.RData")
     )
