...
load(file.path(CRISPRi_RData_directory, "13) Integrate the output from GuideScan for individual sgRNA locations.RData"))
...
are_mapped <- !(is.na(merged_replaced_CRISPRi_df[["Start"]]))
...
sgRNA_polymorphisms_df <- AllPolymorphisms(merged_replaced_CRISPRi_df[are_mapped, ])
...
merged_replaced_CRISPRi_df <- data.frame(merged_replaced_CRISPRi_df,
...
save(list = "merged_replaced_CRISPRi_df",
     file = file.path(CRISPRi_RData_directory, "14) Find overlaps between sgRNA sequences and genetic polymorphisms.RData")
     )
