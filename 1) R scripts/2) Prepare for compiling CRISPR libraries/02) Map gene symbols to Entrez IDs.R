...
# Convert the lists from NCBI's Hs_info file to all capital letters -------
...
# synonyms_to_symbol_list <- NamesToAllCaps(ConsolidateList(synonyms_to_symbol_list))
...
symbols_df <- data.frame("Symbol"            = collected_symbols,
                         ...
                         "Number_OrgHs"      = vapply(collected_symbols, function(x) if (x %in% names(symbol_to_entrez_list)) length(symbol_to_entrez_list[[x]]) else NA_integer_, integer(1)),
                         "Number_all"        = lengths(combined_list),
                         "Entrez_IDs_OrgHs"  = vapply(collected_symbols, function(x) if (x %in% names(symbol_to_entrez_list)) paste0(symbol_to_entrez_list[[x]], collapse = ", ") else NA_character_, ""),
...
unique_OrgHs_symbols_list <- symbol_to_entrez_list[lengths(symbol_to_entrez_list) == 1]
unique_OrgHs_alias_list   <- alias_to_entrez_list[lengths(alias_to_entrez_list) == 1]
unique_NCBI_synonyms_list <- synonyms_to_entrez_list[lengths(synonyms_to_entrez_list) == 1]

intersect_OrgHs_NCBI_list <- sapply(intersect(names(alias_to_entrez_list), names(synonyms_to_entrez_list)),
...
symbols_df[["Unambiguous_alias"]] <- vapply(collected_symbols, function(x) {
  ...
  if (is.null(unique_OrgHs_alias_lookup) || is.null(unique_NCBI_synonyms_lookup)) {
    ...
    unique_symbols <- unique(unlist(c(unique_OrgHs_alias_lookup, unique_NCBI_synonyms_lookup), use.names = FALSE))
...
symbols_assignment_map <- c(
  "Unique_in_OrgHs_symbol" = "Only one mapping in org.Hs.egSYMBOL",
  "Present_in_NCBI_symbol" = "No unique mapping in org.Hs.egSYMBOL, but found in the Symbol column of Homo_sapiens.gene_info",
  "Unambiguous_alias"      = "One and only one mapping in both org.Hs.egALIAS and Homo_sapiens.gene_info",
  "Unique_in_overlap"      = "Only one mapping that is common to both org.Hs.egALIAS and Homo_sapiens.gene_info",
  "Unique_in_OrgHs_alias"  = "Only present in org.Hs.egALIAS, and only one mapping found",
  "Unique_in_NCBI_synonym" = "Only present in Homo_sapiens.gene_info, and only one mapping found",
  "Min_OrgHs_symbol"       = "Maps to multiple IDs in org.Hs.egSYMBOL; the first (lowest) ID was chosen",
  "Min_OrgHs_alias"        = "Maps to multiple IDs in org.Hs.egALIAS; the first (lowest) ID was chosen",
