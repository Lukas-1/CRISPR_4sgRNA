...
OrderControlsVec <- function(CRISPR_df) {
  ...
  if (is_CRISPRko) {
    ...
    controls_vec[are_Brunello_controls] <- as.integer(sub("^Control_", "", CRISPR_df[["Combined_ID"]][are_Brunello_controls]))
  } else {
    controls_vec <- NA_vec
  }
  return(controls_vec)
...
RankCRISPRDfByTSS <- function(CRISPR_df, legacy_mode = FALSE) {
  if (legacy_mode) {
    are_superfluous_TSS <- (CRISPR_df[["Num_TSSs"]] > 1) & is.na(CRISPR_df[["TSS_ID"]])
  } else {
    are_superfluous_TSS <- CRISPR_df[, "Is_superfluous_TSS"]
  }
...
RankCRISPRDf <- function(CRISPR_df, reorder_by_rank = TRUE, allow_5pG_MM = FALSE, ID_column = "Combined_ID") {
  ...
  CRISPR_df <- CRISPR_df[pre_order, , drop = FALSE]
