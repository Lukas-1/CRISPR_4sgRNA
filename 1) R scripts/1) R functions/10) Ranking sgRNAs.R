...
RankDf <- function(CRISPR_sub_df, allow_5pG_MM = FALSE) {
  ...
  SNP_data_present <- (preferred_AF_max_column %in% colnames(CRISPR_sub_df))
  ...
  my_order <- order(CRISPR_sub_df[, "Is_control"] == "No",
                    ...
                    if ("CRISPOR_4MM_specificity" %in% colnames(CRISPR_sub_df)) !(is.na(CRISPR_sub_df[, "CRISPOR_4MM_specificity"]))          else NA_vec,
                    ...
                    if ("CRISPOR_Doench_efficacy" %in% colnames(CRISPR_sub_df)) CRISPR_sub_df[, "CRISPOR_Doench_efficacy"]                    else NA_vec,
                    if ("hCRISPRa_v2_rank" %in% colnames(CRISPR_sub_df)) -(suppressWarnings(as.integer(CRISPR_sub_df[, "hCRISPRa_v2_rank"]))) else NA_vec,
                    if ("Calabrese_rank" %in% colnames(CRISPR_sub_df)) -(match(CRISPR_sub_df[, "Calabrese_rank"], c("1/2/3", "4/5/6")))       else NA_vec,
