...
load(file.path(rdata_dir, "01_read_in_data_chunk7.RData"))
...
extracted_df <- chunk7_fastq_df[, c("Sequence_sg2", "Sequence_sg3")]
...
chunk7_matched_df <- do.call(data.frame, c(list(chunk7_fastq_df[, !(names(chunk7_fastq_df) %in% c("Quality_sg2", "Quality_sg3"))]),
                                           extract_df_list,
...
save(chunk7_matched_df,
     file = file.path(rdata_dir, "03_look_up_sgRNAs_chunk7.RData")
     )
