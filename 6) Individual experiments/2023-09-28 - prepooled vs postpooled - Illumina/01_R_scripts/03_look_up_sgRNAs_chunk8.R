...
load(file.path(rdata_dir, "01_read_in_data_chunk8.RData"))
...
extracted_df <- chunk8_fastq_df[, c("Sequence_sg2", "Sequence_sg3")]
...
chunk8_matched_df <- do.call(data.frame, c(list(chunk8_fastq_df[, !(names(chunk8_fastq_df) %in% c("Quality_sg2", "Quality_sg3"))]),
                                           extract_df_list,
...
save(chunk8_matched_df,
     file = file.path(rdata_dir, "03_look_up_sgRNAs_chunk8.RData")
     )
