...
hist((counts_df[, "Sum_MaySwitch_xMM"] - counts_df[, "Sum_NoEvidentSwitch_xMM"]) / counts_df[, "Sum_MaySwitch_xMM"])
...
## -- 33 (0.15%) when tolerating template switches and 1 base mismatch
## -- 34 (0.15%) when tolerating only template switches (zero mismatches allowed)
## -- 61 (0.27%) when including only reads without a template switch (and with two sgRNAs from the library), but tolerating 1 base mismatch
## -- 65 (0.29%) when including only reads without a template switch (and with two sgRNAs from the library) and without any mismatched bases
...
## When considering sg2 and sg3 individually, 35 sg2 sequences (0.16%) and
## 58 sg3 sequences (0.26%) were not found at all in the sequencing data.
## 27 plasmids were represented either only by sg2 or only by sg3.
## For 25 plasmids, only sg2 was found, but not sg3, whereas for 2 plasmids,
...
#### were missing (0.10%), whereas 29 out of 18266 non-essential genes were missing (0.16%).
...
#### When considering just the “T0” timepoint, 2 out of 2056 essential
#### genes were missing (0.10%), whereas 31 out of 18266 non-essential genes
#### were missing (0.17%); this difference did NOT reach statistical
#### significance (odds ratio 0.57, p value = 0.8).
#### For this analysis, template switches and 1 base mismatch were tolerated.
are_present <- rowSums(counts_df[, c("MaySwitch_xMM_Prepool_T0_R1", "MaySwitch_xMM_Prepool_T0_R2")]) != 0
...
## and 841 non-essential genes were represented in the library), only 5
...
## from the "T0" timepoint before introduction of the CRISPRoff system,
