...
screen_data_directory <- file.path(file_input_directory, "Pooled screen", "Data - with non-targeting controls")
...
B1_v_YM5_df <- read.csv(file = file.path(screen_data_directory, "Final_list_YM5_B1.csv"),
                        stringsAsFactors = FALSE, check.names = FALSE
                        )
D5_v_YM5_df <- read.csv(file = file.path(screen_data_directory, "Final_list_YM5_D5.csv"),
                        stringsAsFactors = FALSE, check.names = FALSE
                        )
B1_v_D5_df  <- read.csv(file = file.path(screen_data_directory, "DMSO5--over--B1_DOWN_GeneName.csv"),
                        stringsAsFactors = FALSE, check.names = FALSE
                        )
...
HitStrengthScatter <- function(input_df_1,
  ...
  required_columns <- c("Hit_strength", "Gene_Name", "Gene_symbol")
  ...
  matches_vec <- match(input_df_1[, "Gene_Name"], input_df_2[, "Gene_Name"])
...
VolcanoPlot <- function(input_df, main_title, selection = "Non-targeting") {
  ...
  AddPoints(x_vec, y_vec, input_df[, "Gene_symbol"], selection = selection)
...
AddPoints <- function(x_vec, y_vec, symbols_vec, selection, random_order = TRUE) {
  ...
  if (selection == "Essential") {
    ...
    gene_sets <- list(
      "Essential" = mouse_essential_df[mouse_essential_df[, "Category"] %in% "Essential", "Mouse_symbol"]
    )
    fill_colors_map <- c(
      "Essential" = brewer.pal(9, "Reds")[[6]]
    )
    labels_list <- list(
      "Essential" = c("Essential genes", "in humans", "(n = 2033)")
    )
    ...
    gene_sets <- list(
      "Top 10 overall"       = top_10_HS,
      "Top 10 non-essential" = top_10_NE
    )

    fill_colors_map <- c(
      "Top 10 overall"       = brewer.pal(9, "Reds")[[6]],
      "Top 10 non-essential" = brewer.pal(9, "Blues")[[6]]
    )

    border_colors_map <- c(
      "Top 10 overall"       = brewer.pal(9, "Reds")[[8]],
      "Top 10 non-essential" = brewer.pal(9, "Blues")[[8]]
    )

    labels_list <- list(
      "Top 10 overall"       = c("Top 10 genes", "(overall, by ", "hit strength)"),
      "Top 10 non-essential" = c("Top 10 genes", "(filtered by", "essentiality)")
    )
    ...
    fill_colors_map <- c(
      "Ostm1" = brewer.pal(9, "Oranges")[[6]],
      "Clcn7" = brewer.pal(9, "Blues")[[5]]
    )

    border_colors_map <- c(
      "Ostm1" = brewer.pal(9, "Oranges")[[8]],
      "Clcn7" = brewer.pal(9, "Blues")[[8]]
    )

    labels_list <- list(
      "Ostm1" = expression(bolditalic("Ostm1")),
      "Clcn7" = expression(bolditalic("Clcn7"))
    )
...
AllHitStrengthScatters <- function(input_df_1,
  ...
  selection_names <- c("Non-targeting", "Essential", "Top 10", "Top 2")
...
HitStrengthBeeBox <- function(data_df,
  ...
  numeric_vec <- data_df[use_indices, "Hit_strength"]
  ...
  mtext(group_labels, at = group_positions, side = 1, line = 0.6,
  ...
  for (i in seq_along(symbols_list)) {
    ...
    if (sum(are_this_group) >= 3) {
      ...
      segments(x0  = min(group_positions[are_this_group]),
               ...
               y0  = par("usr")[[3]] - diff(grconvertY(c(0, 1.8), from = "lines", to = "user")),
               col = "gray50",
               lwd = par("lwd"),
               xpd = NA
               )
...
All3BeeBoxes <- function(add_to_file_name, symbols_list = NULL, brewers_vec = NULL, small_label_symbols = NULL) {
  ...
  prefix <- paste0("Hit strength box plot - ", add_to_file_name, " - ")


  png(file.path(beebox_path, paste0(prefix, "D5 vs. YM5", ".png")),
      width = beebox_width, height = beebox_height, units = "in", res = 600
      )
  HitStrengthBeeBox(D5_v_YM5_df, "DMSO vs. YM (day 5)",
  ...
  png(file.path(beebox_path, paste0(prefix, "2) B1 vs. D5", ".png")),
      width = beebox_width, height = beebox_height, units = "in", res = 600
      )
  HitStrengthBeeBox(B1_v_D5_df, "Baseline vs. DMSO day 5",
                    symbols_list = symbols_list,
                    super_groups_brewers = brewers_vec,
                    small_label_symbols = small_label_symbols
                    )
  ...
  png(file.path(beebox_path, paste0(prefix, "3) B1 vs. YM5", ".png")),
      width = beebox_width, height = beebox_height, units = "in", res = 600
      )
  HitStrengthBeeBox(B1_v_YM5_df, "Baseline vs. YM day 5",
                    symbols_list = symbols_list,
                    super_groups_brewers = brewers_vec,
                    small_label_symbols = small_label_symbols
                    )
...
matches_vec <- match(B1_v_D5_df[, "Gene_Name"], B1_v_YM5_df[, "Gene_Name"])
stopifnot(!(anyNA(matches_vec)))

B1_v_D5_df[, "Gene_symbol"] <- B1_v_YM5_df[matches_vec, "Gene_symbol"]
...
ReorderBySequence <- function(input_df) {
  input_df <- input_df[order(input_df[, "Gene_Name"]), ]
  row.names(input_df) <- NULL
  return(input_df)
}

B1_v_YM5_df <- ReorderBySequence(B1_v_YM5_df)
D5_v_YM5_df <- ReorderBySequence(D5_v_YM5_df)
B1_v_D5_df  <- ReorderBySequence(B1_v_D5_df)
...
B1_v_YM5_df[["Hit_strength"]] <- B1_v_YM5_df[, "log2 Ratio"] * (-log10(B1_v_YM5_df[, "pValue"]))
D5_v_YM5_df[["Hit_strength"]] <- D5_v_YM5_df[, "log2 Ratio"] * (-log10(D5_v_YM5_df[, "pValue"]))
B1_v_D5_df[["Hit_strength"]]  <- B1_v_D5_df[, "log2 Ratio"] * (-log10(B1_v_D5_df[, "pValue"]))
...
are_nontargeting <- grepl("non-targeting", B1_v_D5_df[, "Gene_symbol"], fixed = TRUE)
unique_symbols <- unique(B1_v_D5_df[!(are_nontargeting), "Gene_symbol"])
matches_vec <- match(unique_symbols, mouse_essential_df[, "Mouse_symbol"])

categories_vec <- as.character(mouse_essential_df[matches_vec, "Category"])
categories_vec[is.na(categories_vec)] <- "No data"
use_levels <- c(levels(mouse_essential_df[, "Category"]), "No data")
...
AllVolcanoPlots(D5_v_YM5_df,
                folder_name = "1) Volcano plots - D5 vs. YM5",
                main_title = "DMSO vs. YM (day 5)"
                )

AllVolcanoPlots(B1_v_D5_df,
                folder_name = "2) Volcano plots - B1 vs. D5",
                main_title = "Baseline vs. DMSO day 5"
                )

AllVolcanoPlots(B1_v_YM5_df,
                folder_name = "3) Volcano plots - B1 vs. YM5",
                main_title = "Baseline vs. YM day 5"
                )


AllHitStrengthScatters(B1_v_D5_df, D5_v_YM5_df,
                       axis_label_1 = "Hit strength \u2013 B1 vs. D5",
                       axis_label_2 = "Hit strength \u2013 D5 vs. YM5",
                       "4) Hit strength - D5-YM5 against B1-D5"
                       )

AllHitStrengthScatters(B1_v_YM5_df, D5_v_YM5_df,
                       axis_label_1 = "Hit strength \u2013 B1 vs. YM5",
                       axis_label_2 = "Hit strength \u2013 D5 vs. YM5",
                       "5) Hit strength - D5-YM5 against B1-YM5"
                       )

AllHitStrengthScatters(B1_v_YM5_df, B1_v_D5_df,
                       axis_label_1 = "Hit strength \u2013 B1 vs. YM5",
                       axis_label_2 = "Hit strength \u2013 B1 vs. D5",
                       "6) Hit strength - B1-D5 against B1-YM5"
                       )
...
HitStrengthScatter(B1_v_D5_df, D5_v_YM5_df,
                   x_label   = "Hit strength \u2013 B1 vs. DMSO",
                   y_label   = "Hit strength \u2013 D5 vs. YM5",
                   selection = "Non-targeting"
                   )

VolcanoPlot(B1_v_D5_df, "Baseline vs. DMSO day 5", selection = "Non-targeting")
VolcanoPlot(B1_v_D5_df, "Baseline vs. DMSO day 5", selection = "Essential")
VolcanoPlot(B1_v_D5_df, "Baseline vs. DMSO day 5", selection = "Top 10")
VolcanoPlot(B1_v_D5_df, "Baseline vs. DMSO day 5", selection = "Top 2")
...
beebox_width <- 7.5
beebox_height <- 4.2
beebox_path <- file.path(file_output_directory, "Plots", "9) Hit strength box plots")



All3BeeBoxes("top genes")



lysosome_gene_symbols <- list(
  "V-type ATPases"         = c("Atp6v1b2", "Atp6v1d", "Atp6v1e1", "Atp6v1g1", "Atp6v1h"),
  "Other lysosome-related" = c("Mtor", "Rptor", "Atg16l1", "Uba1", "Clcn7", "Ostm1")
)

All3BeeBoxes("lysosome genes",
             symbols_list = lysosome_gene_symbols,
             brewers_vec = c("Greens", "Blues", "Purples"),
             small_label_symbols = lysosome_gene_symbols[[1]]
             )
...
save(list = c("B1_v_YM5_df", "D5_v_YM5_df", "B1_v_D5_df"),
     file = file.path(R_objects_directory, "06) Show differential enrichment or depletion.RData")
     )
