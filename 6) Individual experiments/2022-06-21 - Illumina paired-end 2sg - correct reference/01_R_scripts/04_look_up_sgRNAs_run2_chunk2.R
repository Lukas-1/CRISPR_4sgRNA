...
load(file.path(first_illumina_trial_dir, "03_R_objects", "01_read_in_data_run2_chunk2.RData"))
load(file.path(rdata_dir, "02_annotate_dJR072_CRISPRoff_library.RData"))



# Prepare data frames for input to the CheckGuides function ---------------------------------------

extracted_df <- run2_chunk2_fastq_df[, paste0("Sequence_sg", 1:2)]
...
run2_chunk2_matched_df <- do.call(data.frame, c(list(run2_chunk2_fastq_df[, !(names(run2_chunk2_fastq_df) %in% c("Quality_sg1", "Quality_sg2"))]),
                                                extract_df_list,
...
save(run2_chunk2_matched_df,
     file = file.path(rdata_dir, "04_look_up_sgRNAs_run2_chunk2.RData")
     )
