...
df_list <- lapply(essential_df[, "Entrez_ID"], function(x) {
  if (is.na(x)) {
...
expanded_indices <- rep(seq_len(nrow(essential_df)),
                        vapply(df_list, nrow, integer(1))
                        )

expanded_df <- data.frame(
  essential_df[expanded_indices, 1:5],
  bound_df[, !(names(bound_df) %in% c("Entrez_ID", "Gene_symbol"))],
  essential_df[expanded_indices, 6:ncol(essential_df)],
  stringsAsFactors = FALSE,
  row.names = NULL
)
...
save(list = "expanded_df",
     file = file.path(R_objects_directory, "05) Merge gene essentiality data with 4sg library coordinates.RData")
     )
