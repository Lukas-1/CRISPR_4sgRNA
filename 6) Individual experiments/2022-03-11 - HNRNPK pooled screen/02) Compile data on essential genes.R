...
file_output_directory <- file.path(file_directory, "3) Output", "Essential genes")

essential_directory   <- file.path(file_input_directory, "Essential genes")
...
# Process Brunello library genes ------------------------------------------

all_entrezs <- as.character(unique(base_v_NT_df[, "Entrez_gene_ID"]))
...
write.table(essential_export_df,
            file = file.path(file_output_directory, "Essential_genes_Brunello.tsv"),
...
save(list = "essential_df",
     file = file.path(R_objects_directory, "02) Compile data on essential genes - essential_df.RData")
     )
...
save(list = c("essential_datasets_list", "achilles_depend_df",
     ...
     file = file.path(R_objects_directory, "02) Compile data on essential genes - dependencies data.RData")
     )
