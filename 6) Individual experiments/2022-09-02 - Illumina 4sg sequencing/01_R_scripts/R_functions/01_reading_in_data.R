...
ProcessSamples <- function(all_reads_list) {
  ...
  results_df <- data.frame(
    "Sample_name"    = sapply(sample_name_splits, "[[", 3),
...
MakeFastqDf <- function(all_reads_list) {
  ...
  df_list <- lapply(indices_list, function(x) {
    data.frame(
      ...
      "Sequence_sg2"   = as.character(ShortRead::sread(all_reads_list[[x[[1]]]])),
      "Sequence_sg3"   = as.character(ShortRead::sread(all_reads_list[[x[[2]]]])),
      "Quality_sg2"    = as.character(as(Biostrings::quality(all_reads_list[[x[[1]]]]), "PhredQuality")),
      "Quality_sg3"    = as.character(as(Biostrings::quality(all_reads_list[[x[[2]]]]), "PhredQuality")),
      stringsAsFactors = FALSE
    )
  })
