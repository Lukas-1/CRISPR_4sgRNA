...
load(file.path(s2r3_R_objects_directory, "07) Extract barcode sequences and quality scores.RData"))
load(file.path(s2r3_R_objects_directory, "08) Categorize subsequences of reads aligned to the reference.RData"))
load(file.path(s2r3_R_objects_directory, "09) Characterize contaminations (using aligned reads).RData"))
load(file.path(s2r3_R_objects_directory, "10) Identify and characterize deletions.RData"))
...
save(list = "plates_analysis_list",
     file = file.path(s2r3_R_objects_directory, "11) Process demultiplexed PacBio reads - plates_analysis_list.RData")
     )

save(list = c("ccs3_df_list", "ccs5_df_list", "ccs7_df_list"),
     file = file.path(s2r3_R_objects_directory, "11) Process demultiplexed PacBio reads - ccs_df_lists.RData")
     )
