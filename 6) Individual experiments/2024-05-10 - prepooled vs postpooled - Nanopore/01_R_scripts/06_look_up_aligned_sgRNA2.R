...
first_half_extracted_df <- first_half_extracted_df[, "Sequence_sg2", drop = FALSE]
names(first_half_extracted_df) <- "Aligned_read_sg2"

second_half_extracted_df <- second_half_extracted_df[, "Sequence_sg2", drop = FALSE]
names(second_half_extracted_df) <- "Aligned_read_sg2"
...
matched_sg2_df <- CheckGuides(extracted_df,
                              2,
...
save(matched_sg2_df,
     file = file.path(rdata_dir, "06_look_up_aligned_sgRNA_2.RData")
     )
