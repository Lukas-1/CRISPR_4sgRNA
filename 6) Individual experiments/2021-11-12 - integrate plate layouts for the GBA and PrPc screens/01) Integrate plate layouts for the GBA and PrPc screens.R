...
file_directory          <- file.path(experiments_directory, "2021-11-12 - integrate plate layouts for the GBA and PrPc screens")
file_input_directory    <- file.path(file_directory, "1) Input")
file_output_directory   <- file.path(file_directory, "3) Output")


plate_layout_96_path    <- file.path(file_input_directory, "2021-09-29_96formatplates.xlsx")
long_layout_96_path     <- file.path(file_input_directory, "ACTIVATION LIBRARY- Plate Layout.xlsx")
...
are_to_use <- (!(full_4sg_by_well_df[["Is_obsolete"]] %in% "Yes")) &
              (full_4sg_by_well_df[["Sublibrary_4sg"]] %in% c("Misc / changed TFs", "Transcription factors"))

use_4sg_df <- full_4sg_by_well_df[are_to_use, ]
...
write.csv(export_df,
          file      = file.path(file_output_directory, "CRISPRa TF - all plates.csv"),
          row.names = FALSE,
          na        = ""
          )
...
for (i in seq_along(layouts_384_mat_list)) {
  ...
  write.csv(export_mat,
            file      = file.path(file_output_directory, "12 plates", plate_name),
